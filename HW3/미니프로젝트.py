...
crawler = NaverNewsCrawler(input('수집을 원하는 키워드를 입력하세요: '))

#### 수집한 데이터를 저장할 엑셀 파일명을 input을 이용해 입력받아 ? 부분에 넣으세요
crawler.get_news(input('저장할 파일의 이름을 입력하세요(확장자까지 입력): '))
...
# 행 단위로 입력받은 데이터 추가
ws.append([number, user_name, user_email])
...
안녕하세요. 메일 보내드립니다.
'''
# 한 행씩 읽어서 여러명에게 메일 보내기
for row in ws.iter_rows():
    addr = row[2].value
    send_mail(SMTP_USER, addr, '자동화 파일입니다.', contents, 'sports.xlsx')